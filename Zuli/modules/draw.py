...
@Zuli.on_message(filters.command(["draw","create","imagine","dream"]))
async def draw(_: Zuli, m: t.Message):
    global Database
    prompt = getText(m)
    if prompt is None:
...
@Zuli.on_callback_query(filters.regex(pattern=r"^d.(.*)"))
async def selectModel(_:Zuli,query:t.CallbackQuery):
    global Database
